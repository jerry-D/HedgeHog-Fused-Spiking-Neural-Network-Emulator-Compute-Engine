...
always @(*)
        case(RM)
            NEAREST : if (((GRS==3'b100) && intFinalq[0]) || (GRS[2] && |GRS[1:0])) roundit = 1'b1;    //when GRS = (3'b100 && lsb) OR when GRS = 101 or 110 or 111 then round it
                      else roundit = 1'b0;
            POSINF  : if (~intFinalq[15] && |GRS) roundit = 1'b1;
                      else roundit = 1'b0;
            NEGINF  : if (intFinalq[15] && |GRS) roundit = 1'b1;
                      else roundit = 1'b0;
            ZERO    : roundit = 1'b0;
        endcase
