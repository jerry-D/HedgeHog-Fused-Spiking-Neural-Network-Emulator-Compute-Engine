...
always @(*)
        case(RM)
            NEAREST : if (((GRS==3'b100) && binOut[0]) || (GRS[2] && |GRS[1:0])) roundit = 1'b1;    //when GRS = (3'b100 && lsb) OR when GRS = 101 or 110 or 111 then round it
                      else roundit = 1'b0;
            POSINF  : if (~binOut[15] && |GRS) roundit = 1'b1;
                      else roundit = 1'b0;
            NEGINF  : if (binOut[15] && |GRS) roundit = 1'b1;
                      else roundit = 1'b0;
            ZERO    : roundit = 1'b0;
        endcase
...
always @(posedge CLK) begin
    if (wren_del_6) RAMA[wraddrs_del_6] <= binOut + roundit;
end 
